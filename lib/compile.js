...
export default class Compile {
	...
	watch() {
		...
		let rebuild = debounce(() => {
			...
			let full = contains(f, ".sfignore");
			watcher.emit("rebuild", f);

			(full ? this._build_source() : this._update_files(f)).then((r) => {
				watcher.emit("build", r, full);
			}).catch((e) => {
		...
		watcher.on("all", (type, full) => {
			...
			if (contains(["add","change","unlink"], type)){
				if (file === ".sfignore" || this.test(file)) {
	...
	_update_files(files, force) {
		return mapSeries(files, (file) => {
			...
			return Promise.all([
				force || server ? this._build_server_entry() : null,
				force || client_styles ? this._concat_styles() : null,
				force || client_scripts ? this._browserify() : null
			]).then(() => res);
		});
	}
