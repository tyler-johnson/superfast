...
function start(argv, log, error) {
	...
	return compile.build().then(function() {
		...
		function sendSeq() {
			if (proc) proc.send("SEQ " + compile.seq);
		}
			...
			p.on("message", onMessage = function(msg) {
				if (msg.match(/^READY/)) {
					...
					if (firstRun) {
						...
						let e = msg.substr(5).trim();
						if (e) {
							e = JSON.parse(e);
							if (e.port) l.push("View it locally at http://localhost:" + e.port);
						}
						log(l.join(" "));
					}
					ready = true;
		...
		compile.on("prebuild", function(full, f) {
			if (!full) log("Rebuilding changed files: %s", f.join(" "));
		});

		compile.on("build", function(seq, res, force) {
			let server, client;
			if (!force) res.some(f => {
