import {omit,debounce,isArray,map,flatten,assign,toArray,includes,last} from "lodash";
		...
		return fs.stat(compiler.cwd).catch((e) => {
			...
			return Promise.all([
				...
				writeJSON(path.join(compiler.cwd, "package.json"), assign({
					...
					dependencies: { "superfast-runtime": "~" + r[1] },
					devDependencies: { superfast: "~" + r[0] }
	...
	pack(options) {
		...
		return readJSON(this.resolve("package.json")).then((pkg) => {
			return this.flush(options, (outputs) => {
				...
				pkgout.main = "server.js";
				pkgout.scripts = { start: "node server.js" };
