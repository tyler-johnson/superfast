...
export default class Compile extends EventEmitter {
	...
	pack(options) {
		...
		return Promise.all([
			...
			.then(() => this._update_ignore_file())
			.then(() => Compile.scandir(this.cwd, this.ignore()))
			.then((f) => this._update_files(this.s.includes.concat(f))),
	...
	// removes everything associated with Superfast
	// there is no going back after this method
	destroy() {
	...
	static create(d, options) {
		options = options || {};
		let compiler = new Compile(d, options);
		let full = compiler.source();
		...
		return fs.stat(full).catch(function(e) {
			...
			return Promise.all([
				...
				writeJSON(resolve(full, "package.json"), assign({
					name: options.name || basename(full),
					version: "0.0.0"
